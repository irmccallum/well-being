...
r2_join$brks <- cut(r2_join$Unlit_rural_pc, 
...
p1 <- ggplot()+ 
  ...
  geom_polygon(data=r2_join, aes(x=long, y=lat,group=group,fill=Unlit_rural_pc), colour="grey50", size = 0.21) +
...
new <- new[order(-new$Unlit_rural_pc),]
...
p2<-ggplot(data=final, aes(x=reorder(id,Unlit_rural_pc), y=Unlit_rural_pc,fill=Unlit_rural_pc)) +
...
ggsave(paste("./figs/figure1_globe_rural.png",sep=""), width=10, height=4, dpi=300)
