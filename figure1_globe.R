...
p3<-ggplot(data=final, aes(x=reorder(id,Unlit_rural_pc), y=Unlit_rural_pc,fill=Unlit_rural_pc)) +
  geom_bar(stat="identity")+
  coord_flip()+
  theme_pubr()+
  theme(text = element_text(size=10))+
...
# save plot
g <- ggarrange(p1,p2,p3,ncol=3,labels = c("a","b","c"),widths = c(2,1,1))
ggsave(paste("./figs/figure1_globe.png",sep=""), width=10, height=4, dpi=300)
